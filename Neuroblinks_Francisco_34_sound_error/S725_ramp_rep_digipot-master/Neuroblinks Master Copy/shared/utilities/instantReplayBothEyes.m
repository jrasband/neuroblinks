function instantReplayBothEyes(data,metadata,data_eye_2,metadata_eye_2)
% Note: This version only currently works with single stim modalities

if isempty(data)||isempty(data_eye_2)
    % Die gracefully
...
% %         stim(400:449,500:549,1,:)=255;
    ...
    case {'conditioning','electrocondition'}
        ...
        cchan(2)=2;
        stfrm{3}=round((metadata_eye_2.cam_eye_2.time(1)+metadata_eye_2.stim.c.isi_eye_2)./1000.*metadata.cam_eye_2.fps);
        enfrm{3}=round(metadata_eye_2.stim.c.usdur_eye_2/1000.*metadata.cam_eye_2.fps); % for second US
        cchan(3)=1;
...
%     F(i).cdata=repmat(data(:,:,:,i),[1 1 3 1])+stim(:,:,:,i); 
    ...
    F(i).colormap=[];
    F_eye_2(i).cdata=repmat(data_eye_2(:,:,:,i),[1 1 3 1]); 
    F_eye_2(i).colormap=[]; 
end
...
if exist('stfrm','var')
    ...
    for j=1:length(stfrm),
        for i=stfrm{j}:stfrm{j}+enfrm{j}
            ...
            F_eye_2(i).cdata(round(m*400/480)+[1:box_size],round(n*550/640)+[(j-1)*box_size+1:j*box_size],setdiff([1,2,3],cchan(j)))=0;
            F_eye_2(i).cdata(round(m*400/480)+[1:box_size],round(n*550/640)+[(j-1)*box_size+1:j*box_size],cchan(j))=255;
        end
    end
end
...
total_F=F;
for i=1:length(total_F)
    total_F(i).cdata=[total_F(i).cdata F_eye_2(i).cdata]; 
end

hm=figure;
set(hm,'Name','Instant Replay - close window when done','Position',[150 150 2*n m]);
movie(hm,total_F,1,20);
