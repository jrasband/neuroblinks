function varargout = ThreshWindowWithPuff2(varargin)
...
% Last Modified by GUIDE v2.5 04-Mar-2021 10:48:48

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ThreshWindowWithPuff2_OpeningFcn, ...
                   'gui_OutputFcn',  @ThreshWindowWithPuff2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function ThreshWindowWithPuff2_OpeningFcn(hObject, eventdata, handles, varargin)
...
handles.x1=floor(metadata.cam_eye_2.winpos(1))+1;
handles.x2=floor(metadata.cam_eye_2.winpos(1)+metadata.cam_eye_2.winpos(3));
handles.y1=floor(metadata.cam_eye_2.winpos(2))+1;
handles.y2=floor(metadata.cam_eye_2.winpos(2)+metadata.cam_eye_2.winpos(4));

set(handles.edit_eyelidThresh,'String',num2str(round(metadata.cam_eye_2.thresh*256)));
set(handles.slider_eyelidThresh,'Value',round(metadata.cam_eye_2.thresh*256));
...
function varargout = ThreshWindowWithPuff2_OutputFcn(hObject, eventdata, handles) 
...
if thres_slider>0 && thres_slider<1,
    metadata.cam_eye_2.thresh=round(get(hObject,'Value'))/256;
...
% Alternatively can use "thresh = graythresh(image)" to get threshold with
% Otsu's method
metadata.cam_eye_2.thresh=str2double(get(handles.edit_eyelidThresh,'String'))/256;
set(handles.slider_eyelidThresh,'Value',round(metadata.cam_eye_2.thresh*256));
...
metadata=getappdata(0,'metadata');
fprintf('thresh = %d.\n', round(metadata.cam_eye_2.thresh*256))
delete(handles.figure2);
...
metadata=getappdata(0,'metadata');
data=getappdata(0,'calb_data_eye_2');

% --- eyelid trace --
[trace,t]=vid2eyetrace_2(data,metadata,metadata.cam_eye_2.thresh);
ind_t=find(t<0.2);
[y_max, ind_max1]=max(trace(ind_t));  [y_min, ind_min1]=min(trace(ind_t));
ind_max=ind_t(ind_max1);  ind_min=ind_t(ind_min1);  
set(0,'CurrentFigure',ghandles.threshgui_eye_2)
...
binframe=im2bw(roi((handles.y1:handles.y2)-1,(handles.x1:handles.x2)-1),metadata.cam_eye_2.thresh);
% binframe=im2bw(roi(handles.y1:handles.y2,handles.x1:handles.x2),metadata.cam.thresh);

handles.image=imshow(roi((handles.y1:handles.y2)-1,(handles.x1:handles.x2)-1),'Parent',handles.axes_image_2);
handles.binimage=imshow(binframe,'Parent',handles.axes_binary_2);

set(ghandles.threshgui_eye_2,'CurrentAxes',handles.axes_hist_2)
imhist(roi(metadata.cam_eye_2.mask))
...
% Had to revise this to work with elliptical ROI
roi=wholeframe.*uint8(metadata.cam_eye_2.mask);
binframe=im2bw(roi(handles.y1:handles.y2,handles.x1:handles.x2),metadata.cam_eye_2.thresh);
handles.image2=imshow(roi(handles.y1:handles.y2,handles.x1:handles.x2),'Parent',handles.axes_image2_2);
handles.binimage2=imshow(binframe,'Parent',handles.axes_binary2_2);

set(ghandles.threshgui_eye_2,'CurrentAxes',handles.axes_hist2_2)
imhist(roi(metadata.cam_eye_2.mask))

% --- for axex_trace: eyelid trace ---
set(ghandles.threshgui_eye_2,'CurrentAxes',handles.axes_trace_2)
plot(t,trace)
...
metadata.cam_eye_2.calib_offset=calib_offset;  metadata.cam_eye_2.calib_scale=calib_scale;
setappdata(0,'metadata',metadata);

fprintf('calib_offset = %d.  calib_scale = %d.\n',calib_offset, calib_scale)
fprintf('thresh = %d.\n', round(metadata.cam_eye_2.thresh*256))

videoname=sprintf('%s\\%s_calib_2',metadata.folder,metadata.TDTblockname);
